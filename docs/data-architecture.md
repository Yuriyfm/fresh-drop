...
Хранит discovery metadata о том, в каких Spotify markets был найден релиз.
...
* `market` описывает только market Spotify, а не страну артиста или релиза;
* при повторном обнаружении релиза в том же market обновляется `last_seen_at`;
* при обнаружении релиза в новом market добавляется новая строка;
* эти данные не должны влиять на пользовательскую country-фильтрацию.
...
Нельзя использовать `market=US`, `market=TR` и другие Spotify markets как `country` артиста или релиза.
...
Для crawler ingestion дополнительно:

* пользовательский `/api/releases` читает только из PostgreSQL;
* search discovery может идти по нескольким markets;
* дубли релизов между markets должны дедуплицироваться до дорогого artist enrichment;
* таблица `artists` используется как cache для enrichment;
* cache refresh артиста может регулироваться TTL, например `SPOTIFY_ARTIST_CACHE_TTL_DAYS=30`.
